...
SYSTEM_PROMPT = \"\"\"You are the Aligner Coach AI, created by Dr. Ajay Kubavat (MDS Orthodontics), Founder of Sure Align Orthodontix n Dentistry, Ahmedabad.
You are a friendly, empathetic expert aligner-treatment assistant.
DETECT the patient language automatically and REPLY in the SAME language.

=== CLINICAL PROTOCOLS ===
WEAR TIME: 20-22 hours/day. Remove ONLY for eating/drinking.
CLEANING: Rinse with lukewarm water; brush gently. NO hot water.
STORAGE: Always in the provided case.
EACH SET: 7-14 days as prescribed by Dr. Ajay Kubavat.

=== EMERGENCY ===
Severe pain, allergic reaction, or lost attachments.
Contact: Dr. Ajay Kubavat | WhatsApp: +916358822642
Clinic: Sure Align Orthodontix n Dentistry, Ahmedabad

ALWAYS end with: 'For any concerns, WhatsApp Dr. Ajay Kubavat: +916358822642'
\"\"\"
...
def tts(text, lang_code):
    ...
    try:
        ...
        r = requests.post(\"https://api.sarvam.ai/text-to-speech\", headers={\"api-subscription-key\": SARVAM_API_KEY, \"Content-Type\": \"application/json\"}, 
                          json={\"text\": text[:1500], \"target_language_code\": tts_lang, \"speaker\": \"anushka\", \"model\": \"bulbul:v2\", \"enable_preprocessing\": True}, timeout=30)
...
def chat(user_msg, history):
    ...
    try:
        r = requests.post(\"https://api.sarvam.ai/v1/chat/completions\", headers={\"Authorization\": f\"Bearer {SARVAM_API_KEY}\", \"Content-Type\": \"application/json\"},
                          json={\"model\": \"sarvam-m\", \"messages\": msgs, \"temperature\": 0.7, \"max_tokens\": 512}, timeout=30)
...
if audio is not None:
    ...
    if audio_id != st.session_state.last_audio_id:
        ...
        if len(wav_bytes) > 1000:
            with st.spinner(\"🎧 Transcribing...-v2.5\"):
                transcript = stt(wav_bytes)
                if transcript: st.session_state.v_input = transcript
        else: st.warning(\"🔇 Recording too short.\")

for m in st.session_state.history:
    with st.chat_message(\"user\"): st.write(m[\"user\"])
    with st.chat_message(\"assistant\", avatar=\"🦷\"):
        st.write(m[\"bot\"])
        if m.get(\"audio\"): st.audio(m[\"audio\"], format=\"audio/wav\")

inp = st.chat_input(f\"Ask in {lang}...\")
if st.session_state.get(\"v_input\"): inp = st.session_state.pop(\"v_input\")
...
if inp:
    ...
    with st.chat_message(\"assistant\", avatar=\"🦷\"):
        st.write(rep)
        if audio_out: st.audio(audio_out, format=\"audio/wav\")
